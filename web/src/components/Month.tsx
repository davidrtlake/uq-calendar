...
const Month = ({
  ...
  const { weekRefs } = useContext(CalendarContext)!
  let startDate: Date
  let endDate: Date
  ...
  events.forEach((e, i) => {
    ...
    if (startDate.getDate() == endDate.getDate()) {
      ...
      while (remainingTime > 0) {
        ...
        eELength = remainingTime < distanceToNextSunday ? remainingTime : distanceToNextSunday
        ...
        if (toggle) {
          for (let i = currDate; i < currDate + eELength - 1; i++) {
            invisExtendedEvents[i] = invisExtendedEvents[currDate - 1] + extendedEvents[currDate - 1].length
            if (publicHoliday) {
        ...
        {Array(Math.ceil((monthLength + fillerDays.length) / 7) * 2)
          ...
          .map((_, row) => {
            ...
            if (row % 2 == 0) {
              ...
              for (let i = dayCount; i < Math.min(dayCount + 7, monthLength); i++) {
            ...
            return Array(8)
              ...
              .map((_, col) => {
                ...
                if (row % 2 == 0) {
                  ...
                  if (col == 0) {
                    return (
                      <div
                        key={col}
                        id={`${monthNum}-${row}`}
                        ref={weekRefs.get(`${yearName}-${monthName}-${row}`)}
                        ...
                        style={{
                          ...
                          paddingTop:
                            row == 0 && fillerDays.length > 0
                              ? "0px"
                              : `${Math.min(2, extendedEvents[dayCount].length) * 25}px`,
                          fontFamily: "Impact, Haettenschweiler, 'Arial Bold', sans-serif",
                          fontWeight: "bold",
                      ...
                      >
                        {!Number.isNaN(parseInt(monthLabels[labelIndex])) ? (
                          <div
                            style={{
                              fontSize: row == 0 && fillerDays.length > 0 ? "12px" : "16px",
                              marginBottom: row == 0 && fillerDays.length > 0 ? "12px" : "20px"
                            }}
                          >
                            WEEK
                          </div>
                        ...
                        <div
                          style={{
                            ...
                            marginBottom: !Number.isNaN(parseInt(monthLabels[labelIndex])) ? "20px" : "auto",
                            marginLeft:
                              parseInt(monthLabels[labelIndex]) == 1 || parseInt(monthLabels[labelIndex]) == 7
                                ? "4px"
                                : "auto"
                          }}
                        >
                  ...
                  } else if ((row == 0 && col - 1 < startDay) || dayCount >= monthLength) {
                    // If first row or after last day, fill with filler days.
                    return (
                      <div key={col} className={styles.fillerDay} style={{ color: "rgb(146, 146, 146)" }}>
                    ...
                    return (
                      <div
                        ...
                        ref={todayMonth && dayCount + 1 == today.getDate() ? todayRef : null}
                        style={{
